...
foreach ($packages as $pkg) {
    $stmt = $pdo->prepare("SELECT AVG(rating) as avg_rating, COUNT(*) as total_reviews 
                           FROM feedback WHERE package_id = ? AND moderation_status = 'published'");
  ...
  <div class="packages-container">
    <?php foreach($packages as $pkg): ?>
      ...
      <div class="package-card">
          <button
            type="button"
            class="favorite-toggle favorite-toggle-card<?= isset($favoriteIds['package'][(int)$pkg['package_id']]) ? ' is-favorite' : '' ?>"
            data-favorite-type="package"
            data-favorite-id="<?php echo (int)$pkg['package_id']; ?>"
            aria-label="Add to favorites"
            aria-pressed="<?= isset($favoriteIds['package'][(int)$pkg['package_id']]) ? 'true' : 'false' ?>"
          ><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20.4c-5.2-3.4-8.4-6.2-8.4-10a4.8 4.8 0 0 1 8.4-3.1 4.8 4.8 0 0 1 8.4 3.1c0 3.8-3.2 6.6-8.4 10Z"></path></svg></button>
            ...
            <div class="package-carousel" data-images="<?php echo htmlspecialchars(json_encode($images, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT), ENT_QUOTES, 'UTF-8'); ?>">
              <button class="prev"><img src="img/prevchevron.png" alt="Previous"></button>
              <img class="carousel-main" src="<?php echo htmlspecialchars((string)$images[0], ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars((string)$pkg['package_title'], ENT_QUOTES, 'UTF-8'); ?>">
          ...
          <a href="package_details.php?package_id=<?php echo (int)$pkg['package_id']; ?>" class="package-link">
            ...
            <div class="package-info">
              <div class="package-title-row">
                ...
                <span class="package-type-badge">
                  <?php echo htmlspecialchars(ucfirst(str_replace('-', ' ', (string)$pkg['package_type'])), ENT_QUOTES, 'UTF-8'); ?>
                </span>
              </div>
        ...
        if ($guides):
            foreach($guides as $guide): ?>
                <div class="guide-card">
                    <button
                      type="button"
                      class="favorite-toggle favorite-toggle-card<?= isset($favoriteIds['guide'][(int)$guide['guide_id']]) ? ' is-favorite' : '' ?>"
                      data-favorite-type="guide"
                      data-favorite-id="<?php echo (int)$guide['guide_id']; ?>"
                      aria-label="Add to favorites"
                      aria-pressed="<?= isset($favoriteIds['guide'][(int)$guide['guide_id']]) ? 'true' : 'false' ?>"
                    ><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20.4c-5.2-3.4-8.4-6.2-8.4-10a4.8 4.8 0 0 1 8.4-3.1 4.8 4.8 0 0 1 8.4 3.1c0 3.8-3.2 6.6-8.4 10Z"></path></svg></button>
                    <div class="guide-left">
                        <img src="<?php echo htmlspecialchars(resolveImage($guide['profile_picture']), ENT_QUOTES, 'UTF-8'); ?>"
                            alt="<?php echo htmlspecialchars((string)$guide['fullname'], ENT_QUOTES, 'UTF-8'); ?>">
                    </div>
...
<div class="content-section" id="our-boats" style="display: none;">
  <div class="boats-wrapper">
    ...
    <div class="boats-list" id="boatsList">
      ...
      ?>
      <div class="boat-card" data-boat="<?php echo htmlspecialchars(json_encode([
            "boat_id"=>$b['boat_id'],
            "name"=>$b['name'],
        ...
        <button
          type="button"
          class="favorite-toggle favorite-toggle-card<?= isset($favoriteIds['boat'][(int)$b['boat_id']]) ? ' is-favorite' : '' ?>"
          data-favorite-type="boat"
          data-favorite-id="<?php echo (int)$b['boat_id']; ?>"
          aria-label="Add to favorites"
          aria-pressed="<?= isset($favoriteIds['boat'][(int)$b['boat_id']]) ? 'true' : 'false' ?>"
        ><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20.4c-5.2-3.4-8.4-6.2-8.4-10a4.8 4.8 0 0 1 8.4-3.1 4.8 4.8 0 0 1 8.4 3.1c0 3.8-3.2 6.6-8.4 10Z"></path></svg></button>
        <div class="boat-card-left">
          <img src="<?php echo htmlspecialchars((string)$imgs[0], ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars((string)$b['name'], ENT_QUOTES, 'UTF-8'); ?>">
        </div>
  ...
  <div class="modal-card">
    ...
    <div class="modal-body">
      ...
      <section class="fee-section">
        ...
        <ul>
          <li>Foreigner <strong>₱<?= number_format($additionalFees['environmental_foreigner']['amount'], 2) ?></strong></li>
          <li>Local <strong>₱<?= number_format($additionalFees['environmental_local']['amount'], 2) ?></strong></li>
          <li>0 – 12 years old <strong>FREE</strong></li>
          <li>Mercedeños <strong>₱<?= number_format($additionalFees['environmental_mercedeno']['amount'], 2) ?></strong></li>
          <li>Senior Citizens <strong>₱<?= number_format($additionalFees['environmental_senior']['amount'], 2) ?></strong></li>
        </ul>
      </section>
      ...
      <section class="fee-section">
        ...
        <ul>
          <li>Apuao Grande <strong>₱<?= number_format($additionalFees['entrance_apuao_grande']['amount'], 2) ?></strong></li>
          <li>Caringo Island <strong>₱<?= number_format($additionalFees['entrance_caringo']['amount'], 2) ?></strong></li>
          <li>Canimog Island (Day Tour) <strong>₱<?= number_format($additionalFees['entrance_canimog_day']['amount'], 2) ?></strong></li>
          <li>Canimog Island (Night Tour) <strong>₱<?= number_format($additionalFees['entrance_canimog_overnight']['amount'], 2) ?></strong></li>
        </ul>
      </section>
      ...
      <section class="fee-section">
        ...
        <ul>
          <li>Apuao Pequeña <strong>₱<?= number_format($additionalFees['docking_apuao_pequena']['amount'], 2) ?></strong></li>
          <li>Malasugui Island <strong>₱<?= number_format($additionalFees['docking_malasugui']['amount'], 2) ?></strong></li>
          <li>Canimog Infinity Pool <strong>₱<?= number_format($additionalFees['docking_canimog']['amount'], 2) ?></strong></li>
        </ul>
      </section>
      ...
      <section class="fee-section">
        ...
        <ul>
          <li>Snorkeling Set (per day) <strong>₱<?= number_format($additionalFees['equipment_snorkeling']['amount'], 2) ?></strong></li>
          <li>Kayak (per day) <strong>₱<?= number_format($additionalFees['equipment_kayak']['amount'], 2) ?></strong></li>
        </ul>
      </section>
