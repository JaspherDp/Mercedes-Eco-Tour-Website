...
function RefundConfirmationEmailTemplate(array $data): string
{
    ...
    $claimActions = [];
    foreach ((array)($data['claim_actions'] ?? []) as $claimAction) {
        if (!is_array($claimAction)) continue;
        $url = trim((string)($claimAction['claim_url'] ?? ''));
        if (!filter_var($url, FILTER_VALIDATE_URL)) continue;
        $claimActions[] = [
            'url' => $e($url),
            'amount' => BookingConfirmationEmailMoney(max(0, (int)($claimAction['amount_minor'] ?? 0)) / 100),
        ];
    }
    if ($claimActions === [] && $claimUrl !== '') {
        $claimActions[] = ['url' => $claimUrl, 'amount' => $amount];
    }
    $hasClaimActions = $claimActions !== [];
    $claimBlocks = '';
    foreach ($claimActions as $index => $claimAction) {
        $claimBlocks .= '<div style="padding:' . ($index === 0 ? '0' : '14px 0 0') . ';' . ($index === 0 ? '' : 'margin-top:14px;border-top:1px solid #cfe6dd;') . '"><strong style="display:block;margin-bottom:7px;color:#164f3e;">Refund ' . ($index + 1) . ': ' . $claimAction['amount'] . '</strong><a href="' . $claimAction['url'] . '" style="display:inline-block;padding:12px 20px;border-radius:9px;background:#176b58;color:#fff;font-weight:700;text-decoration:none;">Claim this refund securely</a></div>';
    }
...
<tr><td style="padding:26px 30px 8px;"><p style="margin:0 0 10px;color:#314f45;font-size:14px;line-height:1.65;">Dear <strong>' . $guest . '</strong>,</p><p style="margin:0 0 18px;color:#314f45;font-size:14px;line-height:1.65;text-align:justify;text-indent:28px;">' . ($manualCompleted ? 'We recorded your approved refund of <strong style="color:#126447;">' . $amount . '</strong> as sent to your verified refund account.' : 'We approved and submitted your refund of <strong style="color:#126447;">' . $amount . '</strong>.' . ($hasClaimActions ? ' PayMongo requires you to complete each claim action below for the corresponding refund amount.' : ' PayMongo will return it to the same account or payment method used for the original transaction.')) . '</p><h2 style="margin:0;padding:0 0 10px;color:#174e3d;font-size:14px;text-transform:uppercase;letter-spacing:.8px;">Refund details</h2><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border:1px solid #dce8e3;border-radius:10px;border-collapse:separate;border-spacing:0;overflow:hidden;background:#fbfdfc;">' . BookingConfirmationEmailRows([
        ...
        'Current status' => $manualCompleted ? 'Sent' : ($hasClaimActions ? 'Waiting for your claim' : 'Processing'),
    ]) . '</table></td></tr>
' . ($hasClaimActions ? '<tr><td style="padding:18px 30px 4px;text-align:center;"><table role="presentation" width="100%" style="border:1px solid #b9dfd1;border-radius:10px;background:#edf9f4;"><tr><td style="padding:18px;color:#315b4e;font-size:13px;line-height:1.55;"><strong style="display:block;margin-bottom:6px;color:#164f3e;">Claim each QR Ph refund within 3 days</strong>Select the bank or e-wallet where PayMongo should send each amount.' . $claimBlocks . '</td></tr></table></td></tr>' : '') . '
...
<tr><td style="padding:20px 30px 28px;"><p style="margin:0;color:#61746d;font-size:13px;line-height:1.6;text-align:justify;text-indent:28px;">' . ($hasClaimActions ? 'Complete every secure PayMongo claim form before its link expires. After each claim, PayMongo will update that refund status automatically.' : 'If the refund has not appeared after the stated period, contact your bank or e-wallet first, then reply to this email so our team can assist you.') . '</p>' . ($trackingUrl !== '' ? '<div style="padding-top:20px;text-align:center;"><a href="' . $trackingUrl . '" style="display:inline-block;padding:12px 20px;border-radius:9px;background:#216d57;color:#fff;font-size:13px;font-weight:700;text-decoration:none;">Track Refund Progress</a><p style="margin:10px 0 0;color:#788981;font-size:10px;line-height:1.5;word-break:break-all;">' . $trackingUrl . '</p></div>' : '') . '</td></tr>
