...
function HoEnsureHotelBookingsTable(PDO $pdo): void
{
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS hotel_room_bookings (
          ...
          checkout_payment_method VARCHAR(30) NULL,
          balance_payment_method VARCHAR(30) NULL,
          created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
          updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    $columnAddSql = [
        ...
        'checkin_id_type' => "ALTER TABLE hotel_room_bookings ADD COLUMN checkin_id_type VARCHAR(60) NULL AFTER checkin_guest_count",
        'checkin_id_reference' => "ALTER TABLE hotel_room_bookings ADD COLUMN checkin_id_reference VARCHAR(255) NULL AFTER checkin_id_type",
        ...
        'checkin_payment_method' => "ALTER TABLE hotel_room_bookings ADD COLUMN checkin_payment_method VARCHAR(30) NULL AFTER checkin_payment_amount",
        ...
        'checkout_payment_method' => "ALTER TABLE hotel_room_bookings ADD COLUMN checkout_payment_method VARCHAR(30) NULL AFTER checkout_final_payment_amount",
        'balance_payment_method' => "ALTER TABLE hotel_room_bookings ADD COLUMN balance_payment_method VARCHAR(30) NULL AFTER checkout_payment_method",
    ];
...
function HoResolveHotelRoomImagePath(?string $path): string
{
    ...
    $webpPath = preg_replace('/\.(?:jpe?g|png|gif)$/i', '.webp', $relativePath);
    if (is_string($webpPath) && $webpPath !== $relativePath) {
        $absoluteWebpPath = $projectRoot . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $webpPath);
...
function HoHydrateHotelRoom(array $row): array
{
    ...
    $mainImage = HoResolveHotelRoomImagePath($row['main_image_path'] ?? null);
    ...
    if (is_array($gallery)) {
        foreach ($gallery as $img) {
            $img = trim((string)$img);
            if ($img !== '') {
                $galleryImages[] = HoResolveHotelRoomImagePath($img);
            }
        }
    }
...
function HoGetUnavailableRoomLookup(PDO $pdo, int $hotelResortId, string $checkin, string $checkout): array
{
    ...
    // A PayMongo booking-time checkout temporarily holds its room while the
    // customer is on the hosted payment page. Drafts never appear as bookings,
    // but honoring the short hold prevents selling the room during payment.
    try {
        if (HoRoomsTableExists($pdo, 'booking_checkout_drafts')) {
            $draftStmt = $pdo->prepare(
                "SELECT
                    CAST(JSON_UNQUOTE(JSON_EXTRACT(payload, '$.hotel_room_id')) AS UNSIGNED) AS hotel_room_id,
                    JSON_UNQUOTE(JSON_EXTRACT(payload, '$.room_type')) AS room_type
                 FROM booking_checkout_drafts
                 WHERE booking_domain = 'hotel' AND status = 'pending' AND expires_at > NOW()
                   AND JSON_UNQUOTE(JSON_EXTRACT(payload, '$.hotel_resort_id')) = :hotel_resort_id
                   AND :checkin <= JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkout_date'))
                   AND :checkout >= JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkin_date'))"
            );
            $draftStmt->execute([
                ':hotel_resort_id' => (string)$hotelResortId,
                ':checkin' => $checkin,
                ':checkout' => $checkout,
            ]);
            foreach ($draftStmt->fetchAll(PDO::FETCH_ASSOC) ?: [] as $row) {
                $roomId = (int)($row['hotel_room_id'] ?? 0);
                if ($roomId > 0) $ids[$roomId] = true;
                $roomType = mb_strtolower(trim((string)($row['room_type'] ?? '')));
                if ($roomType !== '') $names[$roomType] = true;
            }
        }
...
function HoGetAvailableHotelRooms(PDO $pdo, int $hotelResortId, string $checkin, string $checkout, int $guests): array
{
    ...
    foreach ($rooms as $room) {
        ...
        $isBooked = isset($unavailable['ids'][$roomId]) || isset($unavailable['names'][$roomNameKey]);
        $hasCapacity = HoRoomCapacityTotal($room) >= $guests;

        // Mark room with availability status
        $room['is_available'] = !$isBooked && $hasCapacity;
        $room['is_booked_for_dates'] = $isBooked;
        $room['insufficient_capacity'] = !$hasCapacity;

        // Include all rooms (not just available ones)
        $result[] = $room;
    }

    usort($result, static function (array $a, array $b): int {
        // Sort available rooms first
        $aAvail = (int)($a['is_available'] ?? 0);
        $bAvail = (int)($b['is_available'] ?? 0);
        if ($aAvail !== $bAvail) {
                ...
                "SELECT COUNT(*) FROM booking_checkout_drafts
                 WHERE booking_domain = 'hotel' AND status = 'pending' AND expires_at > NOW()
                   ...
                   AND (JSON_UNQUOTE(JSON_EXTRACT(payload, '$.hotel_room_id')) = ?
                        OR JSON_UNQUOTE(JSON_EXTRACT(payload, '$.room_type')) = ?)
                   AND ? <= JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkout_date'))
                   AND ? >= JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkin_date'))"
            );
            $draftStmt->execute([(string)$hotelResortId, (string)$hotelRoomId, $roomName, $checkin, $checkout]);
            if ((int)$draftStmt->fetchColumn() > 0) return false;
        }
    } catch (Throwable $ignored) {
    }
    return true;
...
function HoGetRoomUnavailableDates(
    PDO $pdo,
    int $hotelResortId,
    int $hotelRoomId,
    string $roomName,
    ...
    $stmt = $pdo->prepare("SELECT checkin_date, checkout_date
        FROM hotel_room_bookings
        WHERE hotel_resort_id = :hotel_resort_id
          AND booking_status IN ('pending', 'confirmed')
          AND (hotel_room_id = :hotel_room_id OR (hotel_room_id IS NULL AND room_type = :room_type))
          AND checkin_date <= :range_end
          AND checkout_date >= :range_start");
    $stmt->execute([
        ':hotel_resort_id' => $hotelResortId,
        ':hotel_room_id' => $hotelRoomId,
        ':room_type' => $roomName,
    ...
    try {
        if (HoRoomsTableExists($pdo, 'booking_checkout_drafts')) {
            $draftStmt = $pdo->prepare("SELECT
                    JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkin_date')) AS checkin_date,
                    JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkout_date')) AS checkout_date
                FROM booking_checkout_drafts
                WHERE booking_domain = 'hotel' AND status = 'pending' AND expires_at > NOW()
                  AND JSON_UNQUOTE(JSON_EXTRACT(payload, '$.hotel_resort_id')) = :hotel_resort_id
                  AND (JSON_UNQUOTE(JSON_EXTRACT(payload, '$.hotel_room_id')) = :hotel_room_id
                       OR JSON_UNQUOTE(JSON_EXTRACT(payload, '$.room_type')) = :room_type)
                  AND JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkin_date')) <= :range_end
                  AND JSON_UNQUOTE(JSON_EXTRACT(payload, '$.checkout_date')) >= :range_start");
            $draftStmt->execute([
                ':hotel_resort_id' => (string)$hotelResortId,
                ':hotel_room_id' => (string)$hotelRoomId,
                ':room_type' => $roomName,
                ':range_start' => $from,
                ':range_end' => $to,
            ]);
            $ranges = array_merge($ranges, $draftStmt->fetchAll(PDO::FETCH_ASSOC) ?: []);
        }
    } catch (Throwable $ignored) {
    }
    ...
    foreach ($ranges as $range) {
        $start = DateTimeImmutable::createFromFormat('!Y-m-d', (string)($range['checkin_date'] ?? ''));
        $checkout = DateTimeImmutable::createFromFormat('!Y-m-d', (string)($range['checkout_date'] ?? ''));
        if (!$start || !$checkout || $checkout <= $start) continue;
        $start = $start < $fromDate ? $fromDate : $start;
        $lastReservedDate = $checkout > $toDate ? $toDate : $checkout;
        for ($date = $start; $date <= $lastReservedDate; $date = $date->modify('+1 day')) {
...
function HoGetHotelRoomStatusSnapshot(PDO $pdo, int $hotelResortId): array
{
    ...
    foreach ($rows as $row) {
        ...
        if ($bookingStatus === 'confirmed' && $hasCheckin && !$hasCheckout) {
            ...
            $canCheckin = $bookingStatus === 'confirmed';
        }
        ...
        $booking = [
            ...
            'checkin_id_type' => trim((string)($row['checkin_id_type'] ?? '')),
